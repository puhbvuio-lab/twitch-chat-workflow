...
当前版本：2.0.0。模型通过固定主题代码标注，程序自动生成三级标签；详情见下文“语义标注说明”。
...
主题标注使用固定的 19 个 `topic_code`：模型只选择一个代码，程序自动映射“影响方向、一级模块、二级模块”。Codex 和外部 API 共用完整代码字典，原有标签划分及其他标注字段不变。`game_other` 表示游戏领域内未覆盖的内容，`non_game_other` 表示其他非游戏内容，`undetermined` 表示无法判断；未知代码会明确报错，不会自动归入“其他”。代码映射见 `src/twitch_chat_workflow/models.py` 的 `TOPIC_CODES`。

批次文件、CSV 和 Excel 继续保留中文三级标签，旧的合法标注数据可以继续读取和断点复用。兼容解析器仍接受完整的旧版三级标签响应，但新请求只要求 `topic_code`；同一条响应不能混用两种格式。
